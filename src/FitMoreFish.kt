...
fun main(args : Array<String>) {
    ...
    println(fitMoreFish(10.0, listOf(1,1,1), newFishSize = 6, hasDecorations = true))
...
fun fitMoreFish (tankSize : Double, currentFish : List<Int>, newFishSize : Int = 2 , hasDecorations : Boolean = true) : Boolean {
    return when (hasDecorations) {
        true -> ((tankSize * 80) / 100).toInt() >= (currentFish.sum() + newFishSize)
        false -> tankSize.toInt() >= (currentFish.sum() + newFishSize)
    }
}
